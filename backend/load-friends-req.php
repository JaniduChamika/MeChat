...
$table = DB::search("SELECT `friends`.`id`,`friends`.`user_id` AS `friend_id`,
 `user`.`name` AS `friendname`,`user`.`profile_url` AS `friend_pic`,`user`.`email`,
 `friend_status`.`name` AS `status`
FROM `friends`
INNER JOIN `user` ON `user`.`id`=`friends`.`user_id`
INNER JOIN `friend_status` ON `friends`.`friend_status_id`=`friend_status`.`id`
WHERE `friends`.`freind_id`='$userid' AND `friend_status_id`='2' ORDER BY `user`.`name` ASC");
$friendsArray = array();
...
for ($i = 0; $i < $table->num_rows; $i++) {
      ...
      $friendObj->id = $row["id"];
      $friendObj->friend_id = $row["friend_id"];
